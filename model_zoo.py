...
class TranslationModel(Model_Wrapper):
    ...
    def CharacterModel(self, params):
        """
        Neural machine translation at character level with:
        ...
        #lambda_layer = Lambda(lambda y: y, name='VIEW3', mask = lambda y, m: K.printing(m, "FLATTEN_MASK="))
        #src_embedding = lambda_layer(src_embedding)
        ...
        #for idx, conv in enumerate(outs):
        #    lambda_layer = Lambda(lambda y: y, name='VIEW5-'+str(idx), mask = lambda y, m: K.printing(m, "CONV-ACT_MASK="))
        #    outs[idx]= lambda_layer(outs[idx])
        ...
        for idx, conv in enumerate(outs):
            lambda_layer = Lambda(lambda x: K.permute_dimensions(x, (0, 2, 1, 3)),
                        output_shape=(params['MAX_INPUT_TEXT_LEN'],
                                      ...
                                      params['NUMBER_FILTERS'][idx]),
                        name='permutation_'+str(idx),
                        mask=lambda x, m: m)
        ...
        #for idx, conv in enumerate(outs):
        #    lambda_layer = Lambda(lambda y: y, name='VIEW6-'+str(idx), mask = lambda y, m: K.printing(m, "POOLING_MASK="))
        #    outs[idx]= lambda_layer(outs[idx])
        ...
        #lambda_layer = Lambda(lambda y: K.printing(y, "PRE_CONV_DATA="), name='VIEW1 ')#+ str(idx)
        #lambda_layer.compute_mask = lambda y, m: K.printing(m, "PRE_CONV_MASK=")
        #src_embedding = lambda_layer(src_embedding)
        ...
        #lambda_layer = Lambda(lambda y: y, name='VIEW8', mask = lambda y, m: K.printing(m, "HIGHWAY_MASK="))
        #x = lambda_layer(x)
