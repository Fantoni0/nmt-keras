...
class TranslationModel(Model_Wrapper):
    ...
    def __init__(self, params, model_type='Translation_Model', verbose=1, structure_path=None, weights_path=None,
        ...
        if params['SRC_PRETRAINED_VECTORS'] is not None:
            ...
            self.src_embedding_weights_trainable = params['SRC_PRETRAINED_VECTORS_TRAINABLE'] and params.get(
        ...
        if params['TRG_PRETRAINED_VECTORS'] is not None:
            ...
            self.trg_embedding_weights_trainable = params['TRG_PRETRAINED_VECTORS_TRAINABLE'] and params.get(
    ...
    def AttentionRNNEncoderDecoder(self, params):
        ...
        if params['BIDIRECTIONAL_ENCODER']:
            annotations = Bidirectional(eval(params['ENCODER_RNN_TYPE'])(params['ENCODER_HIDDEN_SIZE'],
                                                                         ...
                                                                         trainable=params.get('TRAINABLE_ENCODER',
                                                                                              True),
        ...
        for n_layer in range(1, params['N_LAYERS_ENCODER']):
            if params['BIDIRECTIONAL_DEEP_ENCODER']:
                current_annotations = Bidirectional(eval(params['ENCODER_RNN_TYPE'])(params['ENCODER_HIDDEN_SIZE'],
                                                                                     kernel_regularizer=l2(params[
                                                                                                               'RECURRENT_WEIGHT_DECAY']),
                                                                                     recurrent_regularizer=l2(params[
                                                                                                                  'RECURRENT_WEIGHT_DECAY']),
                                                                                     bias_regularizer=l2(params[
                                                                                                             'RECURRENT_WEIGHT_DECAY']),
                                                                                     dropout=params[
                ...
                annotations = current_annotations if n_layer == 1 and not params['BIDIRECTIONAL_ENCODER'] else Add()(
                ...
                current_annotations = Regularize(current_annotations, params, name='annotations_' + str(n_layer))
                annotations = current_annotations if n_layer == 1 and params['BIDIRECTIONAL_ENCODER'] else Add()(
                    [annotations, current_annotations])
        ...
        preprocessed_size = params['ENCODER_HIDDEN_SIZE'] * 2 if \
            (params['BIDIRECTIONAL_ENCODER'] and params['N_LAYERS_ENCODER'] == 1) or (
                        params['BIDIRECTIONAL_DEEP_ENCODER'] and params['N_LAYERS_ENCODER'] > 1) \
    ...
    def CharacterModel(self, params):
        """
        Neural machine translation at character level (just the encoder) with:
            * Charater Embedding Layer (See https://arxiv.org/abs/1603.00810 for a detailed explanation.)
            * Attention mechansim on input sequence of annotations
        ...
        # lambda_layer = Lambda(lambda y: y, name='VIEW1', mask=lambda x, m: K.printing(m, "SRC_TEXT_MASK="))
        # src_text = lambda_layer(src_text)

        # 2. Encoder
        # 2.1. Source word embedding. Embedding at character level
        src_embedding = Embedding(input_dim=params['INPUT_VOCABULARY_SIZE'],
                                  output_dim=params['SOURCE_TEXT_EMBEDDING_SIZE'],
                                  name='source_word_embedding',
                                  embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                                  embeddings_initializer=params['INIT_FUNCTION'],
                                  trainable=self.src_embedding_weights_trainable,
        ...
        # lambda_layer = Lambda(lambda y: y, name='VIEW2', mask = lambda y, m: K.printing(m, "SRC_EMBEDDING_MASK="))
        ...
        lambda_layer = Lambda(lambda x: x,
                              ...
                              output_shape=(None,
                                            params['MAX_INPUT_WORD_LEN'],
                                            params['SOURCE_TEXT_EMBEDDING_SIZE']),
                              mask=lambda x, m: K.any(m, axis=2))
        '''
        src_embedding = FlatMask(name='flattenMask', axis=2)(src_embedding)

        ''' 
        Regularize is not a layer. Time Distributed can't be applied to a function.
        src_embedding = Regularize(src_embedding, params, name='src_embedding')
        '''
        # TODO: Problems with TimeDistributed
        ...
        # Convolutions. Filters, length, padding
        outs = []
        total_size = 0
        for nfilters, sizef in zip(params['NUMBER_FILTERS'], params['FILTER_WIDTH']):
            outs.append(TimeDistributed(Conv1D(filters=nfilters, kernel_size=sizef, padding='same'),
                                        name='conv_filter_' + str(sizef))(src_embedding))
            total_size += nfilters
        ...
        x = Concatenate(axis=2, name='concat_layer')(outs)

        # lambda_layer = Lambda(lambda y: y, name='VIEW7', mask = lambda y, m: K.printing(m, "CONCATENATE_MASK="))
        # x = lambda_layer(x)
        ...
        if params['BIDIRECTIONAL_ENCODER']:
            ...
            annotations = eval(params['ENCODER_RNN_TYPE'])(params['ENCODER_HIDDEN_SIZE'],
                                                           ...
                                                           recurrent_initializer=params['INNER_INIT'],
                                                           return_sequences=True,
                                                           name='encoder_' + params['ENCODER_RNN_TYPE'])(x)
        annotations = Regularize(annotations, params, name='annotations')
        ...
        # 3.1.2. Target word embedding
        state_below = Embedding(input_dim=params['OUTPUT_VOCABULARY_SIZE'],
                                output_dim=params['TARGET_TEXT_EMBEDDING_SIZE'],
                                name='target_word_embedding',
                                embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                                embeddings_initializer=params['INIT_FUNCTION'],
    ...
    def Char2Char(self, params):
        ...
        Neural Machine translation at fully character level with:
            * Character embeddings and segment embeddings through convolutions (See https://arxiv.org/abs/1610.03017)
            * Attention mechansim on input sequence of annotations
            * Conditional LSTM for decoding
            * Feed forward layers:
                + Context projected to output
        ...
        # 1.0 Source text Input. Bidimensional, we don't need the concept of words. Just a plain sequence of characters
        src_text = Input(name=self.ids_inputs[0],
        ...
        list_conv = []
        for nfilters, sizef in zip(params['NUMBER_FILTERS'], params['FILTER_WIDTH']):
            conv_out = Conv1D(filters=nfilters,
                              kernel_size=sizef,
                              padding='same',
                              activation='relu')(src_embedding)
            list_conv.append(conv_out)

        # 2.3 Concatenate list of convolutions outputs into a single tensor
        conv = Concatenate(axis=2)(list_conv)
        conv = Lambda(lambda x: K.reshape(x,
                                          tuple([x.shape[0], -1, sum(params['NUMBER_FILTERS'])])))(conv)

        # 2.4 Max Pooling
        outs = MaxPooling1D(pool_size=params.get('POOL_SIZE', 5),
                            padding='valid')(conv)

        # 2.5 Highway Layers
        outs = TimeDistributed(Highway(activation='relu'), name='Highway_1')(outs)
        outs = TimeDistributed(Highway(activation='relu'), name='Highway_2')(outs)
        outs = TimeDistributed(Highway(activation='relu'), name='Highway_3')(outs)
        outs = TimeDistributed(Highway(activation='relu'), name='Highway_4')(outs)

        # 2.6 Get mask for segment vectors
        outs = Lambda(lambda x: x,
                      name='GetMask',
                      mask=lambda x, m: K.any(x, axis=2))(outs)
        # 2.6 Bi-Directional RNN
        if params['BIDIRECTIONAL_ENCODER']:
            annotations = Bidirectional(eval(params['ENCODER_RNN_TYPE'])(params['ENCODER_HIDDEN_SIZE'],
                                                                         kernel_regularizer=l2(
                                                                             params['RECURRENT_WEIGHT_DECAY']),
            ...
            annotations = eval(params['ENCODER_RNN_TYPE'])(params['ENCODER_HIDDEN_SIZE'],
                                                           ...
                                                           recurrent_initializer=params['INNER_INIT'],
                                                           return_sequences=True,
                                                           name='encoder_' + params['ENCODER_RNN_TYPE'])(outs)
        annotations = Regularize(annotations, params, name='annotations')
        # 2.7 Potentially deep encoder
        for n_layer in range(1, params['N_LAYERS_ENCODER']):
            if params['BIDIRECTIONAL_DEEP_ENCODER']:
                current_annotations = Bidirectional(eval(params['ENCODER_RNN_TYPE'])(params['ENCODER_HIDDEN_SIZE'],
                                                                                     kernel_regularizer=l2(
            ...
            # 3.0 Decoder
            # 3.1 Previously generated words as inputs for training -> Teacher forcing
            next_words = Input(name=self.ids_inputs[1], batch_shape=tuple([None, None]), dtype='int32')
            ...
            # 3.3. Attentional decoder
            sharedAttRNNCond = eval('Att' + params['DECODER_RNN_TYPE'] + 'Cond')(params['DECODER_HIDDEN_SIZE'],
                                                                                 att_units=params.get('ATTENTION_SIZE',
                                                                                                      0),
                                                                                 kernel_regularizer=l2(
                                                                                     params['RECURRENT_WEIGHT_DECAY']),
                                                                                 recurrent_regularizer=l2(
    ...
    def Multitask(self, params):
        ...
        MULTITASK MODEL. All parameters but the softmax layer are shared across the outputs.
        ...
        # 3. Decoder
        # 3.1.1. Previously generated words as inputs for training -> Teacher forcing
        # Use characters to see what happens
        next_words = Input(name=self.ids_inputs[1], batch_shape=tuple([None, None]), dtype='int32')
        # 3.1.2. Target word embedding
        state_below = Embedding(params['OUTPUT_VOCABULARY_SIZE'][0], params['TARGET_TEXT_EMBEDDING_SIZE'][0],
                                name='target_word_embedding',
                                embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                                embeddings_initializer=params['INIT_FUNCTION'],
                                trainable=self.trg_embedding_weights_trainable,
                                weights=self.trg_embedding_weights,
                                mask_zero=True)(next_words)
        # state_below = Lambda(lambda x:x, name= 'VIEW_LAYER', mask=lambda x,m: K.printing(x, "THIS_IS_STATE_BELOW= "))(state_below)
        state_below = Regularize(state_below, params, name='state_below')
        ...
        # 3.7.1 Output layer: Softmax
        shared_FC_soft = TimeDistributed(Dense(params['OUTPUT_VOCABULARY_SIZE'][0],
                                               activation=params['CLASSIFIER_ACTIVATION'],
                                               kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                               bias_regularizer=l2(params['WEIGHT_DECAY']),
                                               trainable=params.get('TRAINABLE_DECODER', True),
        ...
        shared_FC_soft2 = TimeDistributed(Dense(out_soft_size,
                                                ...
                                                bias_regularizer=l2(params['WEIGHT_DECAY']),
                                                trainable=params.get('TRAINABLE_DECODER', True),
                                                name=params['CLASSIFIER_ACTIVATION'] + '2'
                                                ),
                                          trainable=params.get('TRAINABLE_DECODER', True),
        ...
        # Softmax
        softout = shared_FC_soft(out_layer)
        softout2 = shared_FC_soft2(out_layer)  # Second Softmax - Character level
    ...
    def MultitaskDecoder(self, params):
        """
        Neural machine translation with:
            * BRNN encoder
            * Attention mechansim on input sequence of annotations
        ...
        MULTITASK MODEL. All parameters but the softmax layer and the decoder are shared across the outputs.
        ...
        # 3.1.1. Previously generated words as inputs for training -> Teacher forcing
        # Use characters
        next_chars = Input(name=self.ids_inputs[1], batch_shape=tuple([None, None]), dtype='int32')
        next_words = Input(name=self.ids_inputs[2], batch_shape=tuple([None, None]), dtype='int32')
        # 3.1.2. Target word embedding
        state_below = Embedding(params['OUTPUT_VOCABULARY_SIZE'][0], params['TARGET_TEXT_EMBEDDING_SIZE'][0],
                                name='target_word_embedding',
                                embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                                ...
                                trainable=self.trg_embedding_weights_trainable,
                                weights=self.trg_embedding_weights,
                                mask_zero=True)(next_chars)
        # word level
        state_below2 = Embedding(params['OUTPUT_VOCABULARY_SIZE'][1], params['TARGET_TEXT_EMBEDDING_SIZE'][1],
                                name='target_word_embedding2',
                                embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                                embeddings_initializer=params['INIT_FUNCTION'],
                                trainable=self.trg_embedding_weights_trainable,
                                weights=self.trg_embedding_weights,
        ...
        state_below2 = Regularize(state_below2, params, name='state_below2')
        ...
        if len(params['INIT_LAYERS']) > 0:
            ...
            initial_state = Regularize(initial_state, params, name='initial_state')
            input_attentional_decoder = [state_below, annotations, initial_state]
            input_attentional_decoder2 = [state_below2, annotations, initial_state]
            ...
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                ...
                initial_memory = Regularize(initial_memory, params, name='initial_memory')
                input_attentional_decoder.append(initial_memory)
                input_attentional_decoder2.append(initial_memory)
        else:
            # Initialize to zeros vector
            input_attentional_decoder = [state_below, annotations]
            input_attentional_decoder2 = [state_below2, annotations]
            initial_state = ZeroesLayer(params['DECODER_HIDDEN_SIZE'])(ctx_mean)
            input_attentional_decoder.append(initial_state)
            input_attentional_decoder2.append(initial_state)
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                input_attentional_decoder.append(initial_state)
                input_attentional_decoder2.append(initial_state)
        ...
        # TODO: Replicate using for bucle for any given number of outputs
        # 3.3. Attentional decoder. Second decoder
        sharedAttRNNCond2 = eval('Att' + params['DECODER_RNN_TYPE'] + 'Cond')(params['DECODER_HIDDEN_SIZE'],
                                                                              attention_mode=params.get(
                                                                                  'ATTENTION_MODE',
                                                                                  'add'),
                                                                              att_units=params.get('ATTENTION_SIZE', 0),
                                                                              kernel_regularizer=l2(
        ...
        # Modify input_attentional_decoder for the second decoder
        rnn_output2 = sharedAttRNNCond2(input_attentional_decoder2)
        proj_h2 = rnn_output2[0]
        x_att2 = rnn_output2[1]
        alphas2 = rnn_output2[2]
        h_state2 = rnn_output2[3]
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            h_memory2 = rnn_output2[4]
        ...
        shared_proj_h_list2 = []
        shared_reg_proj_h_list = []
        shared_reg_proj_h_list2 = []

        h_states_list = [h_state]
        h_states_list2= [h_state2]
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            h_memories_list = [h_memory]
            h_memories_list2 = [h_memory2]

        for n_layer in range(1, params['N_LAYERS_DECODER']):
            current_rnn_input = [proj_h, shared_Lambda_Permute(x_att), initial_state]
            current_rnn_input2 = [proj_h2, shared_Lambda_Permute(x_att2), initial_state]
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                current_rnn_input.append(initial_memory)
                current_rnn_input2.append(initial_memory)
            shared_proj_h_list.append(eval(params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond')(
                params['DECODER_HIDDEN_SIZE'],
                kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                ...
                num_inputs=len(current_rnn_input),
                name='decoder_' + params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond' + str(n_layer)))
            shared_proj_h_list2.append(eval(params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond')(
                params['DECODER_HIDDEN_SIZE'],
                kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                conditional_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
            ...
            current_proj_h = current_rnn_output[0]
            current_proj_h2 = current_rnn_output2[0]
            h_states_list.append(current_rnn_output[1])
            h_states_list2.append(current_rnn_output2[1])
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                h_memories_list.append(current_rnn_output[2])
                h_memories_list2.append(current_rnn_output2[2])
            ...
            proj_h = Add()([proj_h, current_proj_h])
            proj_h2 = Add()([proj_h2, current_proj_h2])
        ...
        shared_FC_mlp = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                        ...
                                        name='logit_lstm')
        out_layer_mlp = shared_FC_mlp(proj_h)
        out_layer_mlp2 = shared_FC_mlp(proj_h2)
        ...
        out_layer_ctx = shared_FC_ctx(x_att)
        out_layer_ctx = shared_Lambda_Permute(out_layer_ctx)
        out_layer_ctx2 = shared_FC_ctx(x_att2)
        out_layer_ctx2 = shared_Lambda_Permute(out_layer_ctx2)
        shared_FC_emb = TimeDistributed(Dense(params['SKIP_VECTORS_HIDDEN_SIZE'],
                                              kernel_initializer=params['INIT_FUNCTION'],
                                              kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                              bias_regularizer=l2(params['WEIGHT_DECAY']),
        ...
        out_layer_emb2 = shared_FC_emb(state_below2)

        [out_layer_mlp2, shared_reg_out_layer_mlp2] = Regularize(out_layer_mlp2, params,
                                                                 shared_layers=True, name='out_layer_mlp2')
        [out_layer_ctx2, shared_reg_out_layer_ctx2] = Regularize(out_layer_ctx2, params,
                                                                 shared_layers=True, name='out_layer_ctx2')
        [out_layer_emb2, shared_reg_out_layer_emb2] = Regularize(out_layer_emb2, params,
                                                               shared_layers=True, name='out_layer_emb2')

        shared_additional_output_merge = eval(params['ADDITIONAL_OUTPUT_MERGE_MODE'])(name='additional_input')
        additional_output = shared_additional_output_merge([out_layer_mlp, out_layer_ctx, out_layer_emb])
        additional_output2 = shared_additional_output_merge([out_layer_mlp2, out_layer_ctx2, out_layer_emb2])
        shared_activation = Activation(params.get('SKIP_VECTORS_SHARED_ACTIVATION', 'tanh'))

        out_layer = shared_activation(additional_output)

        out_layer2 = shared_activation(additional_output2)
        ...
        shared_deep_list2 = []
        shared_reg_deep_list2 = []
        # 3.6 Optional deep ouput layer
        for i, (activation, dimension) in enumerate(params['DEEP_OUTPUT_LAYERS_2']):
            shared_deep_list2.append(TimeDistributed(Dense(dimension, activation=activation,
                                                          kernel_initializer=params['INIT_FUNCTION'],
                                                          kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                                          bias_regularizer=l2(params['WEIGHT_DECAY']),
                                                          trainable=params.get('TRAINABLE_DECODER', True),
                                                    ...
                                                    trainable=params.get('TRAINABLE_DECODER', True),
                                                    name=activation + '_%d_2' % i))
            out_layer2 = shared_deep_list2[-1](out_layer2)

            [out_layer2, shared_reg_out_layer2] = Regularize(out_layer2,
                                                             params, shared_layers=True,
                                                             name='out_layer2_' + str(activation) + '_%d' % i)
            shared_reg_deep_list2.append(shared_reg_out_layer2)
        ...
        shared_FC_soft2 = TimeDistributed(Dense(params['OUTPUT_VOCABULARY_SIZE'][1],
                                                activation=params['CLASSIFIER_ACTIVATION'],
                                                kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                                bias_regularizer=l2(params['WEIGHT_DECAY']),
                                                trainable=params.get('TRAINABLE_DECODER', True),
                                          ...
                                          trainable=params.get('TRAINABLE_DECODER', True),
                                          name=self.ids_outputs[1])
        softout2 = shared_FC_soft2(out_layer2)

        self.model = Model(inputs=[src_text, next_chars, next_words], outputs=[softout, softout2])

        if params['DOUBLE_STOCHASTIC_ATTENTION_REG'] > 0.:
            self.model.add_loss(alpha_regularizer)
            self.model.add_loss(alpha_regularizer2)
        ##################################################################
        #                         SAMPLING MODEL                         #
        ##################################################################
        # Now that we have the basic training model ready, let's prepare the model for applying decoding
        ...
        # First, we need a model that outputs the preprocessed input + initial h state
        # for applying the initial forward pass
        model_init_input = [src_text, next_chars, next_words]
        model_init_output = [softout, softout2, annotations] + h_states_list + h_states_list2
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            model_init_output += h_memories_list
            model_init_output += h_memories_list2
        if self.return_alphas:
            model_init_output.append(alphas)
            model_init_output.append(alphas2)
        self.model_init = Model(inputs=model_init_input, outputs=model_init_output)
        ...
        input_attentional_decoder = [state_below, preprocessed_annotations,
                                     prev_h_states_list[n_deep_decoder_layer_idx]]
        input_attentional_decoder2 = [state_below2, preprocessed_annotations,
                                     prev_h_states_list2[n_deep_decoder_layer_idx2]]
        ...
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            ...
            input_attentional_decoder.append(prev_h_memories_list[n_deep_decoder_layer_idx])
            input_attentional_decoder2.append(prev_h_memories_list2[n_deep_decoder_layer_idx2])
        # Apply decoder
        rnn_output = sharedAttRNNCond(input_attentional_decoder)
        proj_h = rnn_output[0]
        x_att = rnn_output[1]
        ...
        x_att2 = rnn_output2[1]
        alphas2 = rnn_output2[2]
        h_states_list2 = [rnn_output2[3]]

        if 'LSTM' in params['DECODER_RNN_TYPE']:
            h_memories_list = [rnn_output[4]]
            h_memories_list2 = [rnn_output2[4]]
        for reg in shared_reg_proj_h:
            proj_h = reg(proj_h)
        for reg in shared_reg_proj_h2:
            proj_h2 = reg(proj_h2)
        ...
        for (rnn_decoder_layer2, proj_h_reg2) in zip(shared_proj_h_list2, shared_reg_proj_h_list2):
            n_deep_decoder_layer_idx2 += 1
            input_rnn_decoder_layer2 = [proj_h2, shared_Lambda_Permute(x_att2),
                                       prev_h_states_list2[n_deep_decoder_layer_idx2]]
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                input_rnn_decoder_layer2.append(prev_h_memories_list2[n_deep_decoder_layer_idx2])

            current_rnn_output2 = rnn_decoder_layer2(input_rnn_decoder_layer2)
            current_proj_h2 = current_rnn_output2[0]
            h_states_list2.append(current_rnn_output2[1])  # h_state
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                h_memories_list2.append(current_rnn_output2[2])  # h_memory
            for reg in proj_h_reg2:
                current_proj_h2 = reg(current_proj_h2)
            proj_h2 = Add()([proj_h2, current_proj_h2])
        ...
        for (reg_out_layer_mlp2, reg_out_layer_ctx2, reg_out_layer_emb2) in zip(shared_reg_out_layer_mlp2,
                                                                             shared_reg_out_layer_ctx2,
                                                                             shared_reg_out_layer_emb2):
            out_layer_mlp2 = reg_out_layer_mlp2(out_layer_mlp2)
            out_layer_ctx2 = reg_out_layer_ctx2(out_layer_ctx2)
            out_layer_emb2 = reg_out_layer_emb2(out_layer_emb2)
        ...
        for (deep_out_layer2, reg_list2) in zip(shared_deep_list2, shared_reg_deep_list2):
            out_layer2 = deep_out_layer2(out_layer2)
            for reg2 in reg_list2:
                out_layer2 = reg2(out_layer2)

        # Softmax
        softout = shared_FC_soft(out_layer)
        softout2 = shared_FC_soft2(out_layer2)  # Second Softmax - Character level
        model_next_inputs = [next_chars, next_words, preprocessed_annotations] + prev_h_states_list + prev_h_states_list2
        model_next_outputs = [softout, softout2, preprocessed_annotations] + h_states_list + h_states_list2
        if 'LSTM' in params['DECODER_RNN_TYPE']:
            model_next_inputs += prev_h_memories_list
            model_next_inputs += prev_h_memories_list2
            model_next_outputs += h_memories_list
            model_next_outputs += h_memories_list2
        ...
        # Store inputs and outputs names for model_next
        # first input must be previous word
        self.ids_inputs_next = [self.ids_inputs[1], self.ids_inputs[2]] + ['preprocessed_input']
        # first output must be the output probs.
        self.ids_outputs_next = self.ids_outputs + ['preprocessed_input']
        # Input -> Output matchings from model_init to model_next and from model_next to model_next
        self.matchings_init_to_next = {'preprocessed_input': 'preprocessed_input'}
