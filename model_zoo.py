...
class TranslationModel(Model_Wrapper):
    ...
    def CharacterModel(self, params):
        ...
        for nfilters, sizef in zip(params['NUMBER_FILTERS'], params['FILTER_WIDTH']):
            outs.append(TimeDistributed(Conv1D(filters=nfilters, kernel_size=sizef, padding='same'), name='conv_filter_'+str(sizef))(src_embedding))
            total_size += nfilters
        ...
        # Define inputs
        n_deep_decoder_layer_idx = 0
        preprocessed_annotations = Input(name='preprocessed_input', shape=tuple([None, preprocessed_size])) #NOne!params['MAX_INPUT_TEXT_LEN']
        prev_h_states_list = [Input(name='prev_state_' + str(i),
                                    shape=tuple([params['DECODER_HIDDEN_SIZE']]))
                            for i in range(len(h_states_list))]
