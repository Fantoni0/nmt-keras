def load_parameters():
    ...
    DATA_ROOT_PATH = '/home/fantonio/datasets/%s/%s/lowercased/joint_bpe/' % (TASK_NAME, SRC_LAN+TRG_LAN)  # Path where data is stored
    ...
    # Source -- Target pkl mapping (used for hls
    # euristics 1--2). See utils/build_mapping_file.sh for further info.
    ...
    FILL_CHAR = 'center'                          # Whether we pad the 'end, 'center' or 'start' of the vectors of characters. Meaningful when using Character encodingen using.
    ...
    # Character-Embedding NMT parameters
    # Both lists MUST have the same length.
    FILL_TARGET = 'end'                                  # Fill for the target language.
    #NUMBER_FILTERS = [25, 50, 75, 100, 125, 150, 175]   # Number of convolutional filters to apply in the encoder.
    NUMBER_FILTERS = [50, 100, 150, 200, 200, 200, 200]  # Number of convolutional filters to apply in the encoder.
    FILTER_WIDTH   = [1, 2, 3, 4, 5, 6, 7]               # Size of the filter. Lower or equal that 'MAX_INPUT_WORD_LEN'
    CHAR_BPE = True 
    ...
    MAX_INPUT_WORD_LEN = 12                        # Maximum length of each word in the input sequence. Meaningful for character NMT. Set to 0 otherwise (the whole words will be used)
    ...
    SOURCE_TEXT_EMBEDDING_SIZE = 15               # Source language word embedding size.
    ...
    ENCODER_HIDDEN_SIZE = 420                     # For models with RNN encoder
    ...
    DECODER_HIDDEN_SIZE = 420                     # For models with RNN decoder
